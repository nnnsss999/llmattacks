...
### extractions1/
Folder containing hundreds of hashed Markdown snapshots created by `extract_urls.py`.
Each filename uses the first ten characters of a SHA-256 digest and is indexed in
`extraction_log.json`.

### @extractions1/
Historical extraction directory generated by an earlier version of the tooling.
It contains a single example Markdown file (`e36fd18630.md`) and its own
`extraction_log.json` for demonstration purposes.
